# Deck 6 v1: Hooks — Enterprise Guardrails

**Duration:** 25 minutes | **Lab:** Lab 5
**Day:** 2 — Productivity
...
## Slide 3: The Hook Lifecycle Events

| Event | When | Use For |
|-------|------|---------|
| **PreToolUse** | Before Edit/Write/Bash | **Blocking** — prevent forbidden actions |
| **PostToolUse** | After Edit/Write/Bash | **Validating** — catch and correct violations |
| **PostCompact** | After context compression | **Re-injecting** — restore critical rules lost to compression |

```
PreToolUse  + exit 1 = Claude cannot do the thing
PostToolUse + exit 1 = Claude did it, sees the error, must fix it
PostCompact          = Injects rules back into a compressed session
```

> **Speaker notes:** PreToolUse is a gate. PostToolUse is a validator. PostCompact is the recovery mechanism — when Claude compresses its context window, your critical rules can get summarized away. A PostCompact hook re-injects them verbatim. Use PostCompact for rules you absolutely cannot afford to lose in long sessions (naming conventions, PII rules, security requirements).
...
- `type`: `command` (shell) — receives full tool input as JSON on stdin
- `command`: use `jq` to parse — extract file path, content, etc.
- `exit 1`: block the action
- `exit 0`: allow the action

> **Speaker notes:** The command receives the full tool input as JSON on stdin. Use `jq` to parse it — extract the file path, the content being written, etc. This is the interface between Claude's actions and your enforcement logic. `statusMessage` shows in the Claude Code UI while the hook is running.
...
| PostCompact re-injection | Rules lost to context compression | Quality in long sessions |

> **Speaker notes:** Each of these is a real enterprise problem. Code review catches some of them sometimes. Hooks catch all of them every time. At scale — 50 developers using Claude Code — hooks are the governance layer that makes adoption safe. The PostCompact pattern is especially valuable for Day 4 capstone work where sessions run long.
...
```json
// settings.json  — everyone gets this
{ "hooks": { "PreToolUse": [{ /* schema guard */ }] } }

// settings.local.json  — just for you
{ "hooks": { "PostToolUse": [{ /* my personal formatter */ }] } }
```
...
- PostCompact: Re-injects top 5 rules after compression (resilience)

> **Speaker notes:** They're complementary, not competing. CLAUDE.md handles the 90% — conventions Claude should follow. Hooks handle the 10% — rules Claude must never violate. PostCompact handles the edge case — rules that must survive context compression in long enterprise sessions.
...
*Bonus demo:*
5. Show PostCompact hook output — what gets re-injected after `/compact`

> **Speaker notes:** [DO THIS LIVE. Let the audience see the block happen in real-time. The "aha" moment is watching Claude try, get blocked, read the error message, and self-correct. For the bonus demo, run /compact then show the PostCompact hook firing — it's invisible in normal use, which is the point.]
...
> **Speaker notes:** Give them 2 minutes to think about this question before they start the lab. It anchors the abstract concept in their real-world experience. You'll come back to this question in the Day 2 retro. Fast finishers: add a PostCompact hook that re-injects the top 3 rules from your CLAUDE.md.
