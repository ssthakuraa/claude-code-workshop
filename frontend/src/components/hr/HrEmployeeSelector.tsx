...
export function HrEmployeeSelector({
  ...
  const resolvedPlaceholder = placeholder ?? t('ui.searchEmployee', { ns: 'common' })
    ...
    <div ref={containerRef} className="relative">
      ...
      {value ? (
        ...
        <div className="relative">
          ...
          <input
            ...
            disabled={disabled}
            className={cn(
              'w-full pl-9 pr-3 py-2 text-sm border rounded-md focus:outline-none focus:ring-2 focus:ring-[color:var(--hr-focus-line)]',
              error ? 'border-red-400' : 'border-neutral-300',
      ...
      {open && !value && (
        <div className="absolute z-50 w-full mt-1 bg-white rounded-md border border-neutral-200 shadow-lg max-h-52 overflow-y-auto">
          {loading ? (
            <div className="px-3 py-6 text-center text-sm text-neutral-500">{t('loading', { ns: 'common' })}</div>
          ) : filtered.length === 0 ? (
            <div className="px-3 py-6 text-center text-sm text-neutral-500">{t('ui.noEmployeesFound', { ns: 'common' })}</div>
          ) : (
            filtered.map(emp => (
              <button
                key={emp.id}
