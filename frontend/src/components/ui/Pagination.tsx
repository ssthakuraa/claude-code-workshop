...
export function Pagination({ page, pageSize, total, onPageChange, onPageSizeChange, pageSizeOptions = [10, 20, 25, 50, 100], className }: PaginationProps) {
  ...
  return (
    <div className={cn('flex items-center justify-between gap-4 flex-wrap', className)}>
      <div className="flex items-center gap-2 text-sm text-neutral-500">
        {total > 0 ? `${start}–${end} of ${total}` : t('ui.noResults', { ns: 'common' })}
        {onPageSizeChange && (
          <select
            value={pageSize}
            onChange={e => { onPageSizeChange(Number(e.target.value)); onPageChange(1) }}
            aria-label={t('ui.rowsPerPage', { ns: 'common' })}
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          disabled={page === 1}
          aria-label={t('ui.previousPage', { ns: 'common' })}
        ...
        <button
          ...
          aria-label={t('ui.nextPage', { ns: 'common' })}
          className={cn(btnBase, 'text-neutral-500 hover:bg-neutral-100 disabled:opacity-40 disabled:cursor-not-allowed')}
        >
