...
export function DatePicker({ value, onChange, label, placeholder, error, hint, required, disabled, minDate, maxDate, className }: DatePickerProps) {
  const { t, i18n } = useTranslation(['common'])
  const [open, setOpen] = useState(false)
  const [coords, setCoords] = useState({ top: 0, left: 0 })
  const triggerRef = useRef<HTMLButtonElement>(null)
  const locale = i18n.resolvedLanguage ?? 'en-US'
  const resolvedPlaceholder = placeholder ?? t('ui.selectDate', { ns: 'common' })
  const dateFnsLocale = locale.startsWith('es') ? es : locale.startsWith('fr') ? fr : locale.startsWith('hi') ? hi : enUS
    ...
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      <div className="relative flex items-center">
        ...
        >
          ...
          <span className="flex-1">{displayValue || resolvedPlaceholder}</span>
          {value && (
            <span
...
export function CurrencyInput({ value, onChange, currency = 'USD', label, placeholder, error, hint, required, disabled, className }: CurrencyInputProps) {
  const { t, i18n } = useTranslation(['common'])
  const locale = i18n.resolvedLanguage ?? 'en-US'
  const symbol = new Intl.NumberFormat(locale, { style: 'currency', currency }).formatToParts(0).find(p => p.type === 'currency')?.value ?? '$'
  const resolvedPlaceholder = placeholder ?? t('ui.currencyAmount', { ns: 'common' })
  ...
  return (
    <div className={cn('flex flex-col gap-1', className)}>
      ...
      <input
        ...
        placeholder={t('ui.phoneNumber', { ns: 'common' })}
        disabled={disabled}
        className={cn(
