...
export function Calendar({ events = [], view: controlledView, onViewChange, onEventClick, onDateClick, renderEvent, className }: CalendarProps) {
  ...
  const weekDayLabels = Array.from({ length: 7 }, (_, index) => weekdayFormatter.format(new Date(2024, 5, 2 + index)))
    ...
    <div className={cn('flex flex-col bg-white rounded-lg border border-neutral-200 overflow-hidden', className)}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-200">
        <div className="flex items-center gap-2">
          <button type="button" onClick={goToday} className="px-2.5 py-1 text-xs font-medium rounded-md border border-neutral-300 hover:bg-neutral-50">{t('today', { ns: 'common' })}</button>
          ...
          <span className="text-sm font-semibold text-neutral-900">{monthYearFormatter.format(new Date(year, month, 1))}</span>
        </div>
                  ...
                  {date && (
                    <>
                      ...
                      <div className="mt-1 space-y-0.5">
                        ...
                        {dayEvents.length > 3 && (
                          <div className="text-xs text-neutral-400 pl-1">+{dayEvents.length - 3}</div>
                        )}
                      </div>
                    </>
                  )}
              ...
              return (
                <div key={i} className="border-r border-neutral-100 last:border-r-0">
                  <div className={cn('text-center py-2 border-b border-neutral-100', isToday && 'bg-primary-50')}>
                    <div className="text-xs text-neutral-400">{weekdayFormatter.format(d)}</div>
      ...
      {view === 'day' && (
        <div className="flex-1 p-4">
          ...
          {eventsOnDay(currentDate).length === 0 ? (
            <p className="text-sm text-neutral-400">{t('ui.noEvents', { ns: 'common' })}</p>
          ) : (
              ...
              {eventsOnDay(currentDate).map(ev => (
                ...
                >
                  <div className="flex-1">
                    ...
                    {ev.start && <p className="text-xs opacity-80">{ev.start.toLocaleTimeString(locale, { hour: 'numeric', minute: '2-digit' })}</p>}
                  </div>
                </div>
              ))}
            </div>
