...
export function SearchBar({ value = '', onChange, onSearch, placeholder, suggestions = [], loading, className }: SearchBarProps) {
  const { t } = useTranslation(['common'])
  const [focused, setFocused] = useState(false)
  const [internal, setInternal] = useState(value)
  const inputRef = useRef<HTMLInputElement>(null)
  const wrapperRef = useRef<HTMLDivElement>(null)
  const resolvedPlaceholder = placeholder ?? t('ui.search', { ns: 'common' })
    ...
    <div ref={wrapperRef} className={cn('relative', className)}>
      ...
      {showSuggestions && (
        <div className="absolute top-full mt-1 left-0 right-0 z-50 bg-white rounded-lg shadow-lg border border-neutral-200 py-1 max-h-72 overflow-y-auto">
          {loading && <div className="px-3 py-3 text-sm text-neutral-400 text-center">{t('ui.searching', { ns: 'common' })}</div>}
