...
export const Select = forwardRef<HTMLDivElement, SelectProps>(
  ({ label, options, value, onChange, placeholder, error, hint,
    ...
    const resolvedPlaceholder = placeholder ?? t('ui.select', { ns: 'common' })
            ...
            <span className={cn('truncate', !selected?.length && 'text-neutral-400')}>
              {multi
                ? (selected as string[]).length
                  ...
                  : resolvedPlaceholder
                : selected ? getLabel(selected as string) : resolvedPlaceholder}
            </span>
            ...
            <div className="absolute z-50 mt-1 w-full rounded-md border border-neutral-200 bg-white shadow-md">
              {searchable && (
                <div className="p-2 border-b border-neutral-100">
                  <input
                    ...
                    placeholder={t('ui.search', { ns: 'common' })}
              ...
              <ul className="max-h-60 overflow-auto py-1" role="listbox">
                {displayOptions.length === 0 && (
                  <li className="px-3 py-2 text-sm text-neutral-400">{t('ui.noOptions', { ns: 'common' })}</li>
                )}
